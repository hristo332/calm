...
function transformToChartData(tasks, startDate, endDate, period, periodLabels) {
  ...
  switch (period) {
    case 'day':
      chartData = [{ label: periodLabels[0], date: startDate, MVT: 0, Priority: 0, Personal: 0, Recharge: 0 }];
      break;
    case 'week':
      chartData = periodLabels.map((label, index) => {
        const date = new Date(startDate);
        date.setDate(date.getDate() + index);
        return { label, date: date.toISOString().split('T')[0], MVT: 0, Priority: 0, Personal: 0, Recharge: 0 };
      });
      break;
    case 'month':
      ...
      chartData = periodLabels.map((label, index) => {
        ...
        return { label, startDate: weekStart.toISOString().split('T')[0], endDate: weekEnd.toISOString().split('T')[0], MVT: 0, Priority: 0, Personal: 0, Recharge: 0 };
      });
      break;
    case 'year':
      const year = new Date(startDate).getFullYear();
      chartData = periodLabels.map((label, index) => ({ label, month: index, year, MVT: 0, Priority: 0, Personal: 0, Recharge: 0 }));
      break;
  }
