...
export async function onRequest(context) {
  ...
  try {
    ...
    // Extract tasks list for timer (today's date only)
    const today = dateParam || new Date().toISOString().split('T')[0];
    const tasks = extractTasks(data.results, today);
...
function extractTasks(results, today) {
  return results
    .filter(task => {
      const props = task.properties;
      const taskDate = props?.Date?.date?.start;
      const status = props?.Status?.select?.name;
      // Return all tasks for selected date (not just incomplete)
    ...
    .map(task => {
      ...
      return {
        ...
        actualDuration: props?.['Actual Duration']?.number || 0
      };
    });
}
